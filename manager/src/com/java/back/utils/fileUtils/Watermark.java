...
import com.java.back.utils.Tools;
...
public class Watermark {
	...
	static {
		strFWATERM = Tools.readTxtFile(FWATERM); // 读取文字水印配置
		strIWATERM = Tools.readTxtFile(IWATERM); // 读取图片水印配置
	}
		...
		if (null != strFWATERM && !"".equals(strFWATERM)) {
			...
			if (strFW.length == 5) {
				if ("yes".equals(strFW[0])) {
					pressText(strFW[1].toString(), imagePath, "", 1, Color.RED,
							Integer.parseInt(strFW[2]),
							Integer.parseInt(strFW[3]),
		...
		if (null != strIWATERM && !"".equals(strIWATERM)) {
			...
			if (strIW.length == 4) {
				if ("yes".equals(strIW[0])) {
					pressImage(getClasspath() + "upload/img/" + strIW[1],
							imagePath, Integer.parseInt(strIW[2]),
